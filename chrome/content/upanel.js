...
var upanel_tempfile_name = 'upanel@coreaudience';
var tcbutton;
var tcmeter;
var tcfield;
var icbutton;
    ...
    try {
        ...
        var dataListener = {
            ...
            onStartRequest: function (request, context) {
                ...
                var props = "[onStartRequest] \t context " + context  + " \n ";
            ...
            onStopRequest: function (request, context, status) {
                ...
                try {
                    ...
                    if (this.data.length == 0) {
                        this.proc.error("Check Service IP and port and VPN/connection settings!");
                    } else {
                    ...
                    if (proc != null) {
                        this.proc.error("Exception: " + excc.message);
                        if (excc.stack) {
                            // Firefox
                            var callstack = [];
                            var lines = excc.stack.split("\n");
                            ...
                            callstack.shift();
                            alert(callstack.join("nn"));
                        }

                    } else {
                        this.proc.error("Exception: proc is NULL");
                    }
...
//https://developer.mozilla.org/en-US/docs/Code_snippets/Threads
        ...
        if (newAPI){
            // todo  fix thread-manager for firefox 20
...
//https://developer.mozilla.org/en-US/docs/Code_snippets/Miscellaneous
        ...
        if (proc != null) {
            proc.error("Exception: " + excc.message);
            if (excc.stack) {
                // Firefox
                var callstack = [];
                var lines = excc.stack.split("\n");
            ...
            proc.error("Exception: wrong process");
        }
...
//var dcParser={
//setValue: function setValue(num1, res) {
// var num=4-num1;
// //var box=document.getElementById("qualifiers-dc-field");
// //this.gender="Unknown";
// //box.setAttribute("style","background-color: lightgrey;");
// if (num==1&&res["W"]>prefs.getCharPref("qbuilder.demotr")) {
//   if (res["NAME"]=="Male") {
//     //box.setAttribute("style","background-color: lightblue;");
//     //this.gender="Male ("+res["W"]*100+"%)";
//   } else
//     if (res["NAME"]=="Female") {
//       //box.setAttribute("style","background-color: pink;");
//       //this.gender="Female ("+res["W"]*100+"%)";
//     } else {
//       //box.setAttribute("style","background-color: lightgrey;");
//     }
// }
//}, 
//parseResponse: function parseResponse(str,ident) {
//  var res={};
//  var result="";
//  if (str=="-")
//    return null;
...
//        result=res["NAME"]+": "+res["W"]+"    "+result;
//        this.setValue(count, res);
//        res={};
//        val_flag=true;
//        name="";
//        value="";
...
//  count++;
//  result=res["NAME"]+": "+res["W"]+"    "+result;
//  document.getElementById("qualifiers-demolabel").value=result;
//  this.setValue(count, res);
//  return "";
//}
//};
...
var cookieParser = {
    parseResponseAndUpdateCookie: function parseResponseAndUpdateCookie(str) {
        ...
        for (i = 0; i < parts.length; i++) {
            ...
            // alert('NAME:'+nv[0]+' - VALUE:'+nv[1]);
            // alert('Setting cookie: '+parts[i]);
...
function upanel_loadPrefs(myevent) {
    ...
    try {
        ...
        file.append(upanel_tempfile_name);
        ...
        if (!file.exists()) {
            ...
            if (newAPI) {
                ...
                file.create("ProfD", [upanel_tempfile_name], false);
            }
        ...
        var msg = upanel_tempfile + ' found in Firefox Profile folder!';
        debug(msg);

        userModeler();
    } catch (except) {
        alert("Exception in loadPrefs: " + except);
    }
}
...
//function setCerterProxy() {
//  prefs.setBoolPref("qbuilder.setproxy",document.getElementById("qsettings-setproxy").checked);
//  if (document.getElementById("qsettings-setproxy").checked) {
//    prefs.setIntPref("network.proxy.type", 1);
//    prefs.setCharPref("network.proxy.http", prefs.getCharPref("qbuilder.proxyip"));
//    prefs.setIntPref("network.proxy.http_port", prefs.getCharPref("qbuilder.proxyport"));
//  } else {
//    prefs.setIntPref("network.proxy.type", 0);
//  }
//}

//function setSaveClicks() {
//  prefs.setBoolPref("qbuilder.saveclicks",document.getElementById("qsettings-saveclicks").checked);
//}
...
function getText(doc, excl, ret) {
    try {
        ...
        return "";
        //alert("Exception in getText: "+except);
    }
}
...
function findFrames(doc, out) {
    ...
    try {
        ...
        if (doc.childNodes != null)
            while ((it = doc.childNodes.item(i)) != null) {
                if (it.nodeName == "FRAME") {
                    //alert(it.src+" "+it.name+" "+it.contentDocument);
        ...
        return false;
        //alert("Exception in findFrames: "+except);
    }
}
...
function checkContent() {
    ...
    if (getBrowser.currentURI ){
        debug('currentURI ' + getBrowser.currentURI);
    }
    if (getBrowser.currentURI.spec ){
        debug('currentURI.spec ' + getBrowser.currentURI.spec);
    }
    ...
    if (getBrowser.currentURI.spec.indexOf("about:") === 0 ) {
        debug(' currentURI spec startsWith   about:');
        return false;
    }
    debug('OK!');
    return true;
...
function qualify() {
    ...
    try {
        ...
        alert("Exception in qualify: " + except);
    }
}
...
function classify() {
    ...
    try {
        ...
        alert("Exception in classify: " + excep);
    }
}
...
//  var flag = prefs.getBoolPref("qbuilder.researchmode");
    ...
    try {
        ...
        alert("Exception in mega classify: " + excep);
    }
}
...
function upanel_accept(id, state) {
    ...
    if (user == "") {
        alert("Please specify your name (Settings->User Name)");
        return;
    }
...
function findNebuad(doc, gres, coord) {
    ...
    try {
            ...
            while (true) {
                ...
                if (it != null) {
                    ...
                    if (coord.y + c.y + it.clientHeight > gres.maxy) {
                        ...
                        //alert(it.nodeName+" "+gres.maxy+" "+it.clientHeight);
                    }
                }
        ...
        return;
        //alert("Exception in findGoogle: "+except);
    }
}
        ...
        while (iter.hasMoreElements()) {
            ...
            if (cookie instanceof Components.interfaces.nsICookie) {
                ...
                if ((cookie.host == cookieDomain || cookie.host == "." + cookieDomain || cookie.host == "www." + cookieDomain ) && cookie.name != null
                    ...
                    //alert("2GRABBING: "+cookie.host+"-> "+cookie.name+"="+cookie.value);
...
function userModelerReset() {
    ...
    try {
        ...
        while (iter.hasMoreElements()) {
            var cookie = iter.getNext();
//            Firebug.Console.log("cookie : "+cookie);
            if (cookie instanceof Components.interfaces.nsICookie) {
//	            Firebug.Console.log("cookie.host : "+cookie.host);
